...
const MAX_RESULTS = 3;
// Ticker landing pages ("MU Stock Price, News, Quote & History") aren't news.
const QUOTE_PAGE =
  /stock price|quote|news \||stock news$|\(NASDAQ:[A-Z]+\) \||\(NYSE:[A-Z]+\) \|/i;
  ...
  async (ticker, name) => {
    const response = await fetch("https://api.tavily.com/search", {
      ...
      body: JSON.stringify({
        ...
        topic: "news",
        time_range: "month",
        max_results: MAX_RESULTS * 2, // same credit cost; leaves room to drop quote pages
    ...
    if (!response.ok) {
      throw new Error(
        `Tavily ${response.status}: ${(await response.text()).slice(0, 200)}`,
      );
    }
    const data = await response.json();
    return (data.results ?? [])
      .filter((r) => !QUOTE_PAGE.test(r.title ?? ""))
      .slice(0, MAX_RESULTS)
      .map((r) => ({
        title: r.title,
        url: r.url,
