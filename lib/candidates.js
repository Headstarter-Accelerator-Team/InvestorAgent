// Turns a parsed question into a short list of scored candidate stocks.
import universe from "@/data/universe.json";
import { getFactSheet, getQuotes, resolveTicker, runScreener, searchCompanies } from "@/lib/market";
import { passesFilters } from "@/lib/score";
import { BACKTESTED, scoreWithModel, styleFitsBacktested } from "@/lib/scoring-models";
import { embed, queryStocks } from "@/lib/server";
import { ServiceError } from "@/lib/errors";
import { SECTOR_OPTIONS } from "@/lib/format";

export const SECTORS = SECTOR_OPTIONS;

// Yahoo predefined screeners that match each style (used when there's no theme).
const STYLE_SCREENERS = {
  growth: ["growth_technology_stocks", "undervalued_growth_stocks"],
  value: ["undervalued_large_caps", "undervalued_growth_stocks"],
  speculative: ["aggressive_small_caps", "small_cap_gainers"],
  balanced: ["undervalued_growth_stocks"],
  dividend: [],
  safe: [],
};

const SHORTLIST = 18;
// Market-cap bands for an explicit size request.
const SIZE_RANGES = { small: [3e8, 2e9], mid: [2e9, 1e10], large: [1e10, Infinity] };

// Smallest company considered. The backtested model defaults to its tested
// range ($10B+) unless the user asks for a size.
function minCapFor(size, style, model) {
  if (SIZE_RANGES[size]) return SIZE_RANGES[size][0];
  if (model !== "classic") return BACKTESTED.minMarketCap;
  return style === "speculative" ? 3e8 : 2e9;
}
const FINAL = 5;
// Without a requested sector/theme, cap picks per sector so results
// aren't five versions of the same bet.
const MAX_PER_SECTOR = 2;

const sectorOf = new Map(universe.map((u) => [u.t, u.s]));

function diversify(items, group, limit, maxPer) {
  const counts = new Map();
  const out = [];
  for (const item of items) {
    const key = group(item) ?? item;
    if ((counts.get(key) ?? 0) >= maxPer) continue;
    counts.set(key, (counts.get(key) ?? 0) + 1);
    out.push(item);
    if (out.length === limit) break;
  }
  return out;
}

// Cheap first-pass rank from batch quote data, so only the most promising
// stocks get a full (slower) fact sheet.
function preScore(q, style, model) {
  if (model !== "classic") {
    // Backtested model = ROE + cash-flow yield. Quotes have neither, so use
    // proxies: ROE ~ (price/book) / (price/earnings), and earnings yield.
    const pe = q.trailingPE;
    if (!(pe > 0) || !(q.priceToBook > 0)) return -1;
    return 0.6 * Math.min(q.priceToBook / pe, 1) + 0.4 * Math.min(10 / pe, 1);
  }
  const pe = q.forwardPE ?? q.trailingPE;
  const peScore = pe && pe > 0 ? Math.max(0, 40 - pe) / 40 : 0;
  const rating = q.analystRating ? (5 - q.analystRating) / 4 : 0.4;
  const mom = q.change52w ?? 0;
  const yld = Math.min(q.dividendYield ?? 0, 0.09);
  const cap = Math.log10(q.marketCap ?? 1) / 12;
  switch (style) {
    case "growth":
    case "speculative":
      return rating + Math.max(-0.5, Math.min(mom, 1));
    case "value":
      return peScore * 2 + rating;
    case "dividend":
      return yld * 20 + rating * 0.5 + cap * 0.5;
    case "safe":
      return cap * 2 + yld * 5 + rating * 0.5 - Math.abs(mom) * 0.5;
    default:
      return rating + peScore + Math.max(-0.3, Math.min(mom, 0.5)) * 0.5;
  }
}

// Theme leaders suggested by the intent model, plus the closest semantic
// matches from each namespace. Financial ranking happens later.
async function themePool(theme, seeds = []) {
  const vector = await embed(theme);
  const [rich, broad] = await Promise.all([
    queryStocks(vector, 15, "stocks"),
    queryStocks(vector, 10, "stock-descriptions"),
  ]);
  const matches = [...rich, ...broad]
    .map((m) => m.Ticker)
    .filter((t) => t && t !== "N/A");
  return [...seeds, ...matches];
}

async function scoreTickers(tickers, { style, sector, size, model, allowFunds, applyFilters }) {
  const settled = await Promise.allSettled(tickers.map((t) => getFactSheet(t)));
  const notes = [];
  const scored = [];
  let unavailable = 0; // failures other than "ticker doesn't exist"
  settled.forEach((r, i) => {
    if (r.status === "rejected") {
      if (/not found/i.test(r.reason?.message)) {
        notes.push(`No market data found for ${tickers[i]}.`);
      } else {
        console.error(`fact sheet failed for ${tickers[i]}`, r.reason);
        unavailable++;
      }
      return;
    }
    const fact = r.value;
    if (applyFilters) {
      if (!passesFilters(fact, { style, allowFunds, minCap: minCapFor(size, style, model) })) return;
      if (model !== "classic" && !styleFitsBacktested(fact, style)) return;
      if (sector && fact.sector !== sector) return;
    }
    scored.push({ fact, score: scoreWithModel(fact, style, model) });
  });
  scored.sort((a, b) => (b.score.composite ?? 0) - (a.score.composite ?? 0));
  if (scored.length === 0 && unavailable > 0) {
    throw new ServiceError("Market data is temporarily unavailable. Please try again in a minute.");
  }
  if (unavailable > 0) notes.push(`Market data couldn't be loaded for ${unavailable} stock(s); they were skipped.`);
  return { scored, notes };
}

// Named companies ("Should I buy Nvidia?", "AAPL vs MSFT").
async function namedCandidates(intent) {
  const resolved = await Promise.all(
    intent.companies.slice(0, FINAL).map(async (c) => {
      const upper = c.trim().toUpperCase();
      if (/^[A-Z.]{1,6}$/.test(upper) && universe.some((u) => u.t === upper)) {
        return upper;
      }
      // An empty search returns null; a thrown error means Yahoo is unreachable.
      const found = await resolveTicker(c).catch((error) => {
        console.error("ticker search failed", error);
        throw new ServiceError("Stock search is temporarily unavailable. Please try again in a minute.");
      });
      return found ?? (/^[A-Z.]{1,6}$/.test(upper) ? upper : null);
    })
  );
  const tickers = [...new Set(resolved.filter(Boolean))];
  if (tickers.length === 0) {
    return { candidates: [], notes: [`Couldn't find a stock for: ${intent.companies.join(", ")}.`] };
  }
  const { scored, notes } = await scoreTickers(tickers, {
    style: intent.style,
    model: intent.model,
    applyFilters: false,
  });
  const missing = intent.companies.filter((_, i) => !resolved[i]);
  if (missing.length) notes.push(`Couldn't find a stock for: ${missing.join(", ")}.`);
  return { candidates: scored, notes };
}

// Open-ended discovery ("safe dividend stocks", "best AI stocks").
async function discoverCandidates(intent) {
  const { style, sector, theme, size, model } = intent;
  const pool = new Set();
  const notes = [];

  if (theme) {
    (await themePool(theme, intent.themeTickers).catch((e) => {
      console.error("theme search failed", e);
      notes.push("Thematic search is unavailable right now; screened the full universe instead.");
      return [];
    })).forEach((t) => pool.add(t));
  }
  if (pool.size === 0) {
    universe
      .filter((u) => !sector || u.s === sector)
      .forEach((u) => pool.add(u.t));
    for (const id of STYLE_SCREENERS[style] ?? []) {
      (await runScreener(id).catch(() => [])).forEach((t) => pool.add(t));
    }
  }

  if (size === "small") {
    (await runScreener("aggressive_small_caps").catch(() => [])).forEach((t) => pool.add(t));
    (await runScreener("small_cap_gainers").catch(() => [])).forEach((t) => pool.add(t));
  }
  const minCap = minCapFor(size, style, model);
  const maxCap = SIZE_RANGES[size]?.[1] ?? Infinity;
  const STYLE_FILTER_NOTES = {
    safe: "Low-risk filter applied (1-year volatility at most 30%, beta at most 1.1).",
    value: "Value filter applied (P/E at most 20).",
    growth: "Growth filter applied (revenue growth at least 5%).",
  };
  if (model !== "classic" && STYLE_FILTER_NOTES[style]) {
    notes.push(`${STYLE_FILTER_NOTES[style]} It narrows the backtested ranking but isn't part of the backtest.`);
  }
  if (model !== "classic" && size && size !== "large") {
    notes.push("The backtested model was only tested on companies worth $10B or more; scores for smaller companies are less reliable.");
  }
  // Skip extra share classes (BRK-B, PBR-A): one line per company.
  const allQuotes = await getQuotes([...pool].filter((t) => !t.includes("-"))).catch((error) => {
    console.error("quote screen failed", error);
    throw new ServiceError("Market data is temporarily unavailable. Please try again in a minute.");
  });
  const quotes = allQuotes.filter(
    (q) =>
      q.quoteType === "EQUITY" &&
      q.exchange !== "PNK" && // OTC / pink-sheet listings
      q.price &&
      (q.marketCap ?? 0) >= minCap &&
      (q.marketCap ?? 0) < maxCap &&
      (style !== "dividend" || (q.dividendYield ?? 0) >= 0.015)
  );
  const focused = Boolean(sector || theme || size === "small");
  // Theme leaders always make the shortlist, whatever their pre-score.
  const seeds = new Set(theme ? intent.themeTickers ?? [] : []);
  const ranked = quotes
    .sort((a, b) => preScore(b, style, model) - preScore(a, style, model))
    .map((q) => q.ticker);
  const shortlist = focused
    ? [...ranked.filter((t) => seeds.has(t)), ...ranked.filter((t) => !seeds.has(t))].slice(0, SHORTLIST)
    : model === "classic"
      ? diversify(ranked, (t) => sectorOf.get(t), SHORTLIST, 4)
      : // Style filters drop more candidates in backtested mode; shortlist wider.
        diversify(ranked, (t) => sectorOf.get(t), style === "balanced" ? SHORTLIST : SHORTLIST * 2, 6);

  // Missing data for screener-picked tickers isn't worth surfacing.
  const { scored } = await scoreTickers(shortlist, {
    style,
    sector,
    size,
    model,
    applyFilters: true,
  });
  // For a theme, its leading companies come first (still sorted by score),
  // so "EV stocks" shows EV makers even when they score poorly.
  const themed = [
    ...scored.filter((c) => seeds.has(c.fact.ticker)),
    ...scored.filter((c) => !seeds.has(c.fact.ticker)),
  ];
  const candidates = focused
    ? themed.slice(0, FINAL).sort((a, b) => (b.score.composite ?? 0) - (a.score.composite ?? 0))
    : diversify(scored, (c) => c.fact.sector, FINAL, MAX_PER_SECTOR);
  return { candidates, screened: quotes.length, notes };
}

// Keyword-fallback words are only companies if they're an exact ticker or a
// whole word of the company's name ("Walmart" -> WMT, but "Safe" is not SAFE).
async function strictResolve(word) {
  if (/^[A-Z]{1,5}$/.test(word) && universe.some((u) => u.t === word)) return word;
  const hits = await searchCompanies(word).catch(() => []);
  const w = word.toLowerCase();
  const hit = hits.find(
    (h) => h.quoteType === "EQUITY" && h.name.toLowerCase().split(/[^a-z0-9&-]+/).includes(w)
  );
  return hit?.symbol ?? null;
}

export async function findCandidates(intent) {
  if (intent.fallback && intent.companies?.length) {
    const tickers = (await Promise.all(intent.companies.map(strictResolve))).filter(Boolean);
    intent = { ...intent, companies: [...new Set(tickers)] };
  }
  if (intent.companies?.length) return namedCandidates(intent);
  return discoverCandidates(intent);
}
