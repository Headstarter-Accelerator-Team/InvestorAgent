...
export const getQuotes = unstable_cache(
  async (symbols) => {
    ...
    for (let i = 0; i < symbols.length; i += 150) {
      const batch = await yahooFinance.quote(symbols.slice(i, i + 150), {
        return: "array",
      });
      out.push(...batch);
    }
...
// "nvidia" / "Coca Cola" / "nvda" -> "NVDA" (equities only).
export const resolveTicker = unstable_cache(
  async (text) => {
    const result = await yahooFinance.search(text, {
      quotesCount: 5,
      newsCount: 0,
    });
    const hit = result.quotes.find(
      (q) => q.symbol && ["EQUITY", "ETF"].includes(q.quoteType)
    );
