...
// Latest annual statement figures, defined like the backtest (SEC-style):
// free cash flow = operating cash flow - capex, ROE = net income / equity.
async function annualStatements(symbol) {
  try {
    const rows = await withTimeout(
      yahooFinance.fundamentalsTimeSeries(symbol, {
        period1: new Date(Date.now() - 2 * 365 * 864e5),
        type: "annual",
        module: "all",
      }),
      10_000,
      `Yahoo annual statements ${symbol}`
    );
    const last = rows.at(-1);
    if (!last) return {};
    return {
      annualFreeCashFlow: num(last.freeCashFlow),
      annualNetIncome: num(last.netIncome),
      annualEquity: num(last.stockholdersEquity),
      fiscalYearEnd: last.date ? new Date(last.date).toISOString().slice(0, 10) : null,
    };
  } catch (error) {
    console.warn(`annual statements failed for ${symbol}:`, error.message);
    return {};
  }
}
...
async function buildFactSheet(symbol) {
  ...
  const annualPromise = annualStatements(symbol);
  let summary;
  try {
  ...
  const annual = await annualPromise;
