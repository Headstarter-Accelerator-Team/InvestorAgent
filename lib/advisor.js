...
const analyze = unstable_cache(
  async (question, style, horizon, table, tickers, model, context, macro) => {
...
export async function advise({ question, style: styleOverride, sector: sectorOverride, size: sizeOverride, history: rawHistory }) {
  ...
  for (const model of [MODELS.smart, MODELS.fast]) {
    try {
      analysis = (await analyze(question, intent.style, intent.horizon, table, tickers, model, context, backdrop)).data;
      break;
    } catch (error) {
      console.error(`analysis failed on ${model}`, error.message);
      lastError = error;
      if (error.status !== 429) break; // only rate limits are worth another model
    }
  }
  if (!analysis) {
    notes.push(
