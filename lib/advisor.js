...
export async function parseIntent(question) {
  for (const model of [MODELS.smart, MODELS.fast]) {
    try {
      return await parseIntentWithModel(question, model);
    } catch (error) {
...
const analyze = unstable_cache(
  async (question, style, horizon, table, tickers, model) => {
...
export async function advise({ question, style: styleOverride, sector: sectorOverride, size: sizeOverride }) {
  ...
  for (const model of [MODELS.smart, MODELS.fast]) {
    try {
      analysis = (await analyze(question, intent.style, intent.horizon, table, tickers, model)).data;
      break;
    } catch (error) {
